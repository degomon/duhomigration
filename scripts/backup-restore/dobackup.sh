...
backup_database() {
    ...
    # Perform parallel backup using pg_dump with compression disabled
    pg_dump -U "$PG_USER" -h "$PG_HOST" -p "$PG_PORT" -d "$db_name" -F d -j "$PARALLEL_JOBS" -Z 0 -f "$backup_path" -v
    ...
    # Compress the backup directory using tar and pigz with '-full' suffix
    local final_filename="${backup_path}-full.tar.gz"
...
run_backups() {
    ...
    for db in "${databases[@]}"; do
        echo "Starting FULL backup job for: $db"
        backup_database "$db"
    done
