# Convenciones de Documentación para Agentes de IA

Cualquier nueva documentación creada por un agente de IA debe seguir una convención de nomenclatura estricta para asegurar la consistencia y la trazabilidad.

## Formato de Nombre de Archivo

El formato para los nombres de archivo de documentación es:

`YYYYMMDD-HHmm-nombre-de-documentacion.md`

### Desglose del Formato:

- **`YYYY`**: Año en 4 dígitos (e.g., `2025`).
- **`MM`**: Mes en 2 dígitos (e.g., `11` para Noviembre).
- **`DD`**: Día en 2 dígitos (e.g., `05`).
- **`HH`**: Hora en 2 dígitos, formato de 24 horas (e.g., `14` para 2 PM).
- **`mm`**: Minutos en 2 dígitos (e.g., `30`).
- **`nombre-de-documentacion`**: Un nombre descriptivo y corto en minúsculas, con palabras separadas por guiones.

### Ejemplo

Si se crea un documento que describe el flujo de creación de facturas a las 2:45 PM del 5 de noviembre de 2025, el nombre del archivo debería ser:

`20251105-1445-flujo-creacion-facturas.md`
