/**
CobroBridgeNativeSingle
Proceso para sincronizar legacy_cobro con C_Payment
Solo para legacy_cobro de tipo native
20210804 - First version
@rule: groovy:CobroBridgeNativeSingle
@class: @script:groovy:CobroBridgeNativeSingle
**/
...
for (int i = 0; i < para.length; i++) {
    ...
    else if (name.equals("cobid"))
        cobid = para[i].getParameterAsInt();
    else
...
MOrg org = MOrg.get(A_Ctx, cob.get_ValueAsInt("AD_Org_ID"));
String orgKey = org.getDescription();
int currentLocalID = cob.get_ValueAsInt("local_id");
String resultQuery = "Inicia Proceso de Cobro Individual";
if(currentLocalID<=0){
    // Obtener BP
    ...
    if(null!=bp && locsize>0){
        ...
        resultQuery = infoData;
    }else
        resultQuery = "Cobro " + cobid + " no tiene un Tercero válido";
    
}else{
    resultQuery = "Cobro " + cobid + " ya fue procesado previamente";
}
