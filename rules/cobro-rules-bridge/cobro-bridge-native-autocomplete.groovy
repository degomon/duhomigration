            ...
            if(bank_account_id<=0){
                ...
                if(null==car){
                    ...
                    if(car!=null && null!=inv && saldoSynced.compareTo(BigDecimal.ZERO)>=0){
                        ...
                        " BPID:> " + cob.get_Value("c_bpartner_id") +
                        " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                        log.info(sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Procesa Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: DESEMBOLSO NO HA SIDO PROCESADO " + org.getValue() );
                        if(car!=null) 
