...
for(GenericPO cob in legacyCobroList){
    ...
    BigDecimal saldoSynced = DB.getSQLValueBD(A_TrxName, "select duho_getsaldo_cartera_synced(?)", [cob.get_ValueAsInt("id_cartera")]);
    if(currentLocalID<=0){
        // Obtener BP
        ...
        if(null!=bp && locsize>0){
            ...
            if(bank_account_id<=0){
                log.info("Skipped (Bank Account not found): " +
                    " BPID:> " + cob.get_Value("c_bpartner_id") +
                    " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                A_ProcessInfo.addLog(0, null, null, "Skipped (Bank Account not found): " + currentLocalID + " BPName: " + bp.getName() + " CobroID: " + cob.get_Value("legacy_cobro_id") );
            }else{
                GenericPO car = new Query(A_Ctx, "legacy_cartera", " legacy_cartera_id = ? ", A_TrxName)
                .setParameters([ cob.get_ValueAsInt("id_cartera") ])
                .first();
                if(null==car){
                    log.info("Skipped (Cartera not found): " +
                    " BPID:> " + cob.get_Value("c_bpartner_id") +
                    " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                    A_ProcessInfo.addLog(0, null, null, "Skipped (Cartera not found): " +
                    " BPID:> " + cob.get_Value("c_bpartner_id") +
                    " CobroID >> " +  cob.get_Value("legacy_cobro_id") + " Cartera no encontrada [001]: " + cob.get_Value("id_cartera") + " BPName: " + bp.getName() );
                }else{
                    MInvoice inv = new Query(A_Ctx, "C_Invoice", " c_invoice_id = ? ", A_TrxName)
                    .setParameters([ car.get_Value("local_id") ])
                    .first();
                    if(car!=null && null!=inv && saldoSynced.compareTo(BigDecimal.ZERO)>=0){
                        if(inv.isPaid()){
                            System.out.println(sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Procesa Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: FACTURA PAGADA - NO PROCEDE COBRO " + org.getValue() );
                            A_ProcessInfo.addLog(0, null, null, sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Procesa Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: FACTURA PAGADA - NO PROCEDE COBRO " + org.getValue() + " BPName: " + bp.getName() + " -- local_id " + currentLocalID );    
                        }
                        else{
                        ...
                        if(saldoSynced.compareTo(BigDecimal.ZERO)<0){
                            System.out.println("Skipped (Cartera sobregirada): " +
                            " BPID:> " + cob.get_Value("c_bpartner_id") +
                            " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                            A_ProcessInfo.addLog(0, null, null, "Valor cobro mayor a Saldo Cartera: " + car.get_Value("legacy_cartera_id") + " BPName: " + bp.getName() );
                        }
                        
                        if(null==car){
                            System.out.println("Skipped (Cartera not found): " +
                            " BPID:> " + cob.get_Value("c_bpartner_id") +
                            " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                            System.out.println(sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Skipped Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: Cartera no encontrada " + org.getValue() );
                            A_ProcessInfo.addLog(0, null, null, "Cartera no encontrada: " + cob.get_Value("id_cartera") + " BPName: " + bp.getName() );
                        }

                        if(null==inv){
                            System.out.println("Skipped (Documento Factura no encontrado): " +
                            " BPID:> " + cob.get_Value("c_bpartner_id") +
                            " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
                            System.out.println(sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Skipped Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: Factura no encontrada " + org.getValue() );
                            A_ProcessInfo.addLog(0, null, null, sdf.format(new Date()) + " -> [" + workNumber.toString() + "] Skipped Cobro===>>> [" + workNumber + " de " + legacyCobroList.size() + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() + " ::: Factura no encontrada " + org.getValue() + " BPName: " + bp.getName() + " -- local_id " + currentLocalID );      
                        }
            ...
            System.out.println("Skipped (BP not found): " +
            " BPID:> " + cob.get_Value("c_bpartner_id") +
            " CobroID >> " +  cob.get_Value("legacy_cobro_id"));
            A_ProcessInfo.addLog(0, null, null, "Skipped (BP not found): " + currentLocalID + " BPName: " + bp.getName() + " CobroID: " + cob.get_Value("legacy_cobro_id") );
        }
    }else{
        A_ProcessInfo.addLog(0, null, null, "Skipped (LocalID = ): " + currentLocalID + " BPName: " + bp.getName() + " CobroID: " + cob.get_Value("legacy_cobro_id") ); 
    }
...
result = "Se migraron " + legacyCobroList.size() + " datos de LegacyCobro a C_Payment en " + td.toString();