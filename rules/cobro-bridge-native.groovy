...
List<GenericPO> legacyCobroList = new Query(A_Ctx, "legacy_cobro",
         " (synced != 'Y' or synced is null) and origen = 'native' and abono>0 and ad_org_id = ? ", A_TrxName)
         .setParameters([ orgid ])
		.setOrderBy("created")
		.list();
System.out.println("Saved the query at least");
for(GenericPO cob in legacyCobroList){
    workNumber = workNumber + 1;
    System.out.println("[" + workNumber.toString() + "] Entra a Procesar Cobro===>>> [" + workNumber + "] ==> "+ cob.get_ValueAsInt("legacy_cobro_id") + " Fecha==> " + cob.get_Value("operacion").toString() );
    /* if(workNumber==3) 
        break; */
