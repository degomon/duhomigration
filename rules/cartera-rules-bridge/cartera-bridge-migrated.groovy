...
String dateString = "31/01/2024"; // Replace with your date string
SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
Date seedDate = dateFormat.parse(dateString);
Timestamp fechaCorte = new Timestamp(seedDate.getTime());
...
List<GenericPO> legacyCartera = new Query(A_Ctx, "legacy_cartera",
         " (synced != 'Y' or synced is null) and origen = 'migrado' ", A_TrxName)
        .setOrderBy("created")
        .list();

for (GenericPO car in legacyCartera) {
    workNumber = workNumber + 1;
    System.out.println("Procesando  [" + workNumber + " de " +
     legacyCartera.size() + "] ==> BPID:> " + car.get_Value("c_bpartner_id") +
     " CarteraID >> " +  car.get_Value("legacy_cartera_id"));
    ...
    String sqlBankAccount = "select u.c_bankaccount_id " +
    " from c_bpartner bp  " +
    " inner join cv_ruta ru on bp.cv_ruta_id = ru.cv_ruta_id " +
    " inner join ad_user u on ru.ad_user_id = u.ad_user_id " +
    " where  bp.C_BPartner_ID = ?";
    int c_bankaccount_id = DB.getSQLValue(A_TrxName, sqlBankAccount, [ car.get_Value("c_bpartner_id") ]);
    if (c_bankaccount_id > 0) {
        if (currentLocalID <= 0) {
            ...
            if(null!=bp && locsize>0){
                ...
                // check if bank_account_id is null or empty
                ...
                invoice.setDateAcct(fechaCorte);
                invoice.setDateInvoiced(fechaCorte);
                ...
                mp.setDateAcct(fechaCorte);
                mp.setDateTrx(fechaCorte);
                mp.setC_BankAccount_ID(c_bankaccount_id); 
                mp.setTenderType("X");
                ...
                log.info("Skipped (BP Not found): " + 
                    " BPID:> " + car.get_Value("c_bpartner_id") +
                    " CarteraID >> " +  car.get_Value("legacy_cartera_id") );  
            }
        } else {
