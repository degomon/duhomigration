...
GenericPO car = new Query(A_Ctx, "legacy_cartera",
         " legacy_cartera_id = ? ", A_TrxName)
		.setParameters([ carteraid ])

org = MOrg.get(A_Ctx, car.get_ValueOfColumn('AD_Org_ID'));
if(org==null || orgid==0){
    result = "ERROR: No existe la Org " + orgid;
    return;
}

    System.out.println("Procesando Cartera Single ::: BPID:> " + car.get_Value("c_bpartner_id"));
    ...
    if(currentLocalID<=0){
        ...
        // Invoice + InvoiceLine
            ...
            A_ProcessInfo.addLog(0,null,null,"Procesada Cartera. " 
            + " Cartera: " + car.get_ID() 
...
result = "Se Procesó Cartera: " + car.get_ID()
+ " a C_Invoice";