...
try {
    ...
    // --- INICIO: MODIFICACIÓN PARA PARÁMETROS OBLIGATORIOS ---
    Object orgidParam = A_ProcessInfo.getParameter("orgid");

    // Validar que el parámetro 'orgid' no sea nulo y tenga un valor válido.
    if (orgidParam == null || ((BigDecimal)orgidParam).intValue() <= 0) {
        throw new AdempiereException("❌ ERROR: El parámetro 'Organización' es obligatorio. Por favor, seleccione una organización para ejecutar el proceso.");
    }

    int p_org_id = ((BigDecimal)orgidParam).intValue();
    A_ProcessInfo.addLog(0, null, null, "Procesando para Organización (AD_Org_ID): ${p_org_id}");

    // Construir la cláusula WHERE. El filtro por organización ahora es fijo.
    String whereClause = "(synced != 'Y' OR synced IS NULL) AND origen = 'native' AND aprobado = 'Y' AND AD_Org_ID = ?";
    
    // Ejecutar la consulta
    List<GenericPO> legacyCartera = new Query(A_Ctx, 'legacy_cartera', whereClause, A_TrxName)
        .setParameters([p_org_id])
        .setOrderBy('created')
        .list();
    // --- FIN: MODIFICACIÓN PARA PARÁMETROS OBLIGATORIOS ---


    if (legacyCartera.isEmpty()) {
        result = 'Proceso finalizado. No se encontraron carteras pendientes para la organización seleccionada.'
    ...
    for (int i = 0; i < legacyCartera.size(); i++) {
        ...
        try {
            ...
            invoice.setDateAcct(new Timestamp(fecha.getTime())); 
            invoice.setDateInvoiced(new Timestamp(fecha.getTime()))
            invoice.setC_DocTypeTarget_ID(NOTA_DOCTYPE_ID); 
            invoice.setC_BPartner_ID(bp.get_ID()); 
            invoice.setC_BPartner_Location_ID(loc_id)
            invoice.setC_PaymentTerm_ID(payment_term_id); 
            invoice.setSalesRep_ID(SALES_REP_ID); 
            invoice.setM_PriceList_ID(PRICE_LIST_ID)
            invoice.setIsSOTrx(true); 
            invoice.set_ValueOfColumn('legacy_id', 
            car.get_ValueAsInt('legacy_cartera_id')); 
            invoice.saveEx()
            ...
            mp.setDateAcct(new Timestamp(fecha.getTime())); mp.setDateTrx(new Timestamp(fecha.getTime())); mp.setC_BankAccount_ID(bank_account_id); mp.setTenderType('X'); mp.setPayAmt(monto)
    ...
    result = "Proceso finalizado. Procesados: ${processedCount}, Omitidos/Error: ${skippedCount}."
} catch (Exception e) {
