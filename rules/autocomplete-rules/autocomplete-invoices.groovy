/**
Process for Autocomplete Draft Invoices
20210701 - First version
key: groovy:AutoCompleteDraftInvoices
name: AutoCompleteDraftInvoices

This process autocompletes Invoices in Draft State.
...
List<MInvoice> draftInvoices = new Query(A_Ctx, "C_Invoice",
         " DocStatus = 'DR' AND 'Y' = duho_isperiodopen( DateAcct, C_DocType_ID ) AND AD_Org_ID = ? ", A_TrxName)
		.setParameters([ orgid ])
        .setOrderBy("created")
		.list();
int i = 0;
for(MInvoice mp in draftInvoices){
    i = i+1;
    String quickLog = new StringBuffer("====> Completando Factura ")
        .append(i.toString() + " de " + draftInvoices.size().toString() + " <===== ")
        .toString();
