...
for(GenericPO cli in legacyClientes){
    i = i+1;
    System.out.println("Trying to migrate [" + i.toString() +  "] ");
    String sb = "";
    ...
    GenericPO ruta = new Query(A_Ctx,"cv_ruta", "AD_Org_ID = ? and id_territorio = ? ", A_TrxName)
        .setParameters( [cli.getAD_Org_ID(), cli.get_ValueAsInt("id_territorio")] )
        .first();    
    
    if(null==city)
        throw new AdempiereException("Ciudad no válida para Org:: " + org.getInfo().getFax());
    if(null==ruta)
        throw new AdempiereException("Ruta no es válida para Territorio: " + cli.get_ValueAsInt("id_territorio").toString() );
    ...
    MBPartner bp = new Query(A_Ctx, "C_BPartner", "value = ?", A_TrxName)
    ...
    if(null==bp){
        ...
        bp.set_ValueOfColumn("legacy_id", cli.get_ValueAsInt("legacy_cliente_id"));
